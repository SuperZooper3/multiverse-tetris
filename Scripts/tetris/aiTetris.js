...
class aiTetrus {
  ...
  canMoveLeft(object) {
    ...
    for (let i = 0; i < object.length; i++) {
      ...
      if (column > 0) {
        if (
          this.board[row * 10 + column - 1].box == undefined ||
          object.includes(this.board[row * 10 + column - 1].box) ||
          this.c.includes(this.board[row * 10 + column - 1].box)
        ) {
          count++;
        }
      }
  ...
  aimoveDown(currentObject) {
    ...
    for (let objectSpot = 0; objectSpot < length; objectSpot++) {
      ...
      if (row < 19) {
        if (
          this.board[(row + 1) * 10 + column].box == undefined ||
          currentObject.includes(this.board[(row + 1) * 10 + column].box) ||
          this.c.includes(this.board[(row + 1) * 10 + column].box)
        ) {
          count++;
        }
      }
  ...
  aiMoveUp(object) {
    ...
    for (let i = 0; i < cObject.length; i++) {
      ...
      if (row > 0) {
        if (
          this.board[(row - 1) * 10 + column].box == undefined ||
          cObject.includes(this.board[(row - 1) * 10 + column].box) ||
          this.c.includes(this.board[(row - 1) * 10 + column].box)
        ) {
          count++;
        }
    ...
    if (box.blockType == 1) {
      if (box.rotation == 0 || box.rotation == 2) {
        for (let i = 0; i < object.length; i++) {
          if (box.row < 17) {
            if (
              this.board[(box.row + i) * 10 + box.column].box == undefined ||
              object.includes(
                this.board[(box.row + i) * 10 + box.column].box
              ) ||
              this.c.includes(this.board[(box.row + i) * 10 + box.column].box)
            ) {
              count++;
            }
          }
        ...
        if (box.column < 7) {
          for (let i = 0; i < object.length; i++) {
            if (
              this.board[box.row * 10 + box.column + i].box == undefined ||
              object.includes(this.board[box.row * 10 + box.column + i].box) ||
              this.c.includes(this.board[box.row * 10 + box.column + i].box)
            ) {
              count++;
            }
          }
  ...
  aiSwap3(box, spot1, spot3, spot4, peice1, peice3, peice4, object) {
    if (
      (this.board[spot1].box == undefined &&
        ...
        this.board[spot4].box == undefined) ||
      this.c.includes(this.board[spot1].box) ||
  ...
  addToBoard(currentObject) {
    ...
    for (let objectSpot = 0; objectSpot < length; objectSpot++) {
      board[
        currentObject[objectSpot].row * 10 + currentObject[objectSpot].column
      ].box = currentObject[objectSpot];
    }
    return board;
  }
  // done
  checkForEmptyBelow(copyObject) {
    ...
    for (let objectSpot = 0; objectSpot < length; objectSpot++) {
      if (copyObject[objectSpot].row < 19) {
        let spot =
          copyObject[objectSpot].row * 10 + 10 + copyObject[objectSpot].column;
  ...
  aiSpacesAbove(currentObject) {
    ...
    for (let j = 0; j < currentObject.length; j++) {
      ...
      for (let i = currentObject[j].row - 1; i >= 0; i--) {
        if (
          i >= 0 &&
          (this.board[spot + i * 10].box == undefined ||
            currentObject.includes(this.board[spot + i * 10].box) ||
            this.c.includes(this.board[spot + i * 10].box))
        ) {
          count++;
        } else {
          break;
  ...
  testSpot(currentObject, swap) {
    ...
    for (let z = 0; z < max; z++) {
      ...
      while (this.aiMoveTile(2, currentObjectClone)) {
        ...
        testRows: for (let row = 19 - k; row > 1; row--) {
          for (let j = 0; j < currentObjectClone.length; j++) {
            let spot = currentObjectClone[j].column;
            if (
              this.board[spot + (currentObjectClone[j].row + row) * 10].box !=
              undefined
            ) {
              continue testRows;
            }
          ...
          for (let objectSpot = 0; objectSpot < length; objectSpot++) {
            copyBoard[
              copyObject[objectSpot].row * 10 + copyObject[objectSpot].column
            ].box = copyObject[objectSpot];
          }
          for (let k = 0; k < copyObject.length; k++) {
            if (copyObject[k].row + 1 <= 19) {
              if (
                copyBoard[(copyObject[k].row + 1) * 10 + copyObject[k].column]
                  .box == undefined
              ) {
                emptyB++;
              }
            ...
            if (columns[column][1] < score) {
              ...
              for (let k = 0; k < copyObject.length; k++) {
                if (copyObject[k].row + 1 <= 19) {
                  if (
                    this.board[
                      (copyObject[k].row + 1) * 10 + copyObject[k].column
                    ].box == undefined
                  ) {
                    emptyBelow++;
                  }
  ...
  placeOneObject() {
    ...
    for (let i = 0; i < combinedpossibleSpots.length - 1; i++) {
      if (
        combinedpossibleSpots[i] == undefined ||
        combinedpossibleSpots[i] == null
      ) {
    ...
    for (let i = 0; i < combinedpossibleSpots.length - 1; i++) {
      ...
      let returnValue = this.findPath(
        combinedpossibleSpots[i][2],
        0,
        0,
        0,
        0,
  ...
  findPath(object, depthCount, left, right, rotate, last, lastr, swap) {
    ...
    if (
      ...
      right < 10 &&
      left < 10 &&
      rotate <= currentObject[0].maxRotation
    ) {
      let correctPositions = 0;
      for (let i = 0; i < this.c.length; i++) {
        if (
          this.c[i].row == currentObject[i].row &&
          this.c[i].column == currentObject[i].column
        ) {
          correctPositions++;
        }
      }
